...
const Community = () => {
  ...
  const submitHandler = async (e): Promise<void> => {
    ...
    try {
      const result = await axios.post(
        "https://heypiserver.herokuapp.com/community/search",
        { search }
      );
      setData(result.data);
    } catch (e) {
  ...
  const handlePageClick = async (data): Promise<any> => {
    ...
    try {
      const result = await axios.post(
        `https://heypiserver.herokuapp.com/community/search?page=${data.selected}`,
        { search }
      );
      setData(result.data);
    } catch (e) {
    ...
    const fetch = async (): Promise<void> => {
      try {
        result = await axios.get(
          "https://heypiserver.herokuapp.com/getQuestions"
        );
        setData(result.data);
      } catch (e) {
        console.log(e);
        ...
        <div style={{ minHeight: "90vh" }} className="dark:bg-gray-900 ">
          <div className="flex justify-center py-10">
            <div className="mb-3 xl:w-96">
              <form
                ...
                className="input-group relative flex  items-stretch w-full mb-4 rounded"
              >
                <input
                  type="search"
          ...
          <div>
            ...
            {data.length > 0 ? (
              data.map((d) => (
                <div
                  onClick={() => router.push(`/question/${d._id}`)}
                  key={d._id}
