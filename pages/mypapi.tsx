...
const Myapi = () => {
    ...
    const handlePageClick = async(data):Promise<any> =>{
        setLoading(true)
        try{
          const result = await axios.post(`http://localhost:8000/userp?page=${data.selected}`, {search})
          setData(result.data)
        }catch(e){
          console.log(e)
        }
    ...
    useEffect(():void => {
        ...
        const fetch  = async():Promise<void> =>{
            try{
                 result = await axios.post('http://localhost:8000/userp', {search, name, token})
                 setData(result.data)
            }catch(e){
                console.log(e)
            }
  ...
  return (
    ...
    <div className="flex border-b border-gray-200 space-x-4 dark:border-gray-700">
        <button onClick={()=>router.push('/myapi')} className="h-10 cursor-pointer py-2 -mb-px text-sm text-center text-transparent bg-transparent border-b-2 border-transparent sm:text-base dark:border-transparent dark:text-white whitespace-nowrap focus:outline-none">
            Public
        </button>

        <button className="h-10  py-2 -mb-px text-sm text-center cursor-pointer text-blue-500 bg-transparent border-b-2 border-blue-500 sm:text-base dark:text-blue-500 whitespace-nowrap cursor-base focus:outline-none hover:border-gray-400">
            Private
        </button>
    </div>
        </div>
