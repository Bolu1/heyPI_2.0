...
const Myapi = () => {
  ...
  const handlePageClick = async (data): Promise<any> => {
    ...
    try {
      const result = await axios.post(
        `https://heypiserver.herokuapp.com/userp?page=${data.selected}`,
        { search }
      );
      setData(result.data);
    } catch (e) {
  ...
  useEffect((): void => {
    ...
    const fetch = async (): Promise<void> => {
      try {
        result = await axios.post("https://heypiserver.herokuapp.com/userp", {
          search,
          name,
          token,
        });
          ...
          <div className="flex justify-center py-5">
            <div className="flex border-b border-gray-200 space-x-4 dark:border-gray-700">
              <button
                ...
                className="h-10 cursor-pointer py-2 -mb-px text-sm text-center text-transparent bg-transparent border-b-2 border-transparent sm:text-base dark:border-transparent dark:text-white whitespace-nowrap focus:outline-none"
              >
                Public
              </button>
