...
const Myapi = () => {
  ...
  const handlePageClick = async (data): Promise<any> => {
    ...
    try {
      const result = await axios.post(
        `https://heypiserver.herokuapp.com/see?page=${data.selected}`,
        { search }
      );
      setData(result.data);
    } catch (e) {
  ...
  useEffect((): void => {
    ...
    const fetch = async (): Promise<void> => {
      try {
        result = await axios.post("https://heypiserver.herokuapp.com/seeme", {
          search,
          name,
          token,
        });
        ...
        <div style={{ minHeight: "90vh" }} className="dark:bg-gray-900 ">
          <div className="flex justify-center py-5">
            <div className="flex border-b border-gray-200 space-x-4 dark:border-gray-700">
              ...
              <button
                onClick={() => router.push("/mypapi")}
                className="h-10 cursor-pointer py-2 -mb-px text-sm text-center text-gray-700 bg-transparent border-b-2 border-transparent sm:text-base dark:text-white whitespace-nowrap cursor-base focus:outline-none hover:border-gray-400"
              >
                Private
              </button>
            </div>
