...
const Codes = () => {
    ...
    const fetch = async (): Promise<void> => {
      try {
        result = await axios.get(
          `https://heypiserver.herokuapp.com/page.html/${id}`
        );
        setData(result.data);
      } catch (err) {
        console.log(err);
  ...
  return (
    <div
      ...
      className="dark:bg-gray-900 dark:text-coolGray-100"
    >
      {loading && (
        <div
      ...
      <Layout title="code">
        {data.code ? (
          <>
            <div
              key="._i"
              className="p-6 mb-5 mx-16 mt-4 sm:p-12 bg-gray-100 dark:bg-gray-800 dark:text-coolGray-100"
            >
            ...
            <div
              style={{ overflowY: "hidden", color: "#f8f8f2" }}
              className=" mx-16 mt-4 bg-zinc-700 cursor-pointer "
            >
